...
export default {
  ...
  methods: {
    _authorizeURL() {
      return store.getters.getRoutes.authorize
    },
    ...
    _goThroughPermissions(path, from, roles, original) {
      ...
      //     }
      //   });
      //   this.routeIsValid == true ? "" : (this.$toaster.error(this.$t("routeNotValid")), from ? from.path == '/' ? router.push({ path: "/list-tickets" }) : router.push({ path: from.path }) : "");
      // }
    },
    ...
    _setRoles(to, from, authorityId) {
      Authority.getRoles(authorityId)
        .then(setRoles => {
          ...
          store.commit("setMenusLength", tempMenus.length);
        })
        .catch();
    },
    ...
    _commitRoles(roles) {
      ...
      roles ? store.commit("setRolesLength", roles.length) : store.commit("setRolesLength", undefined);
    },
    ...
    _renewToken(to, from) {
      User.token()
        .then((result) => {          
          this._commitUser(result.data);
          to && from ? this._setRoles(to, from, result.data.authority.id) : "";
        })
        .catch((error) => {
          this._tokenExpired(error);
          this._getLoading(false);
        })
    },
    ...
    _renewParametersVueX(to, from) {
      ...
      store.getters.getRoles ?
        this._thisRouteIsValid(to, from, store.getters.getRoles) :
        store.getters.getUser.authorityId ?
          this._setRoles(to, from, store.getters.getUser.authorityId) :
